...
<div class="erp-card overflow-hidden">
    <div class="flex flex-col gap-4 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <form method="GET" class="relative flex-1 sm:max-w-sm">
            ...
            <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('pages.search.count') }}" class="erp-input !mt-0 pl-10">
        </form>
    ...
    <div class="overflow-x-auto">
        <table class="erp-table min-w-full">
            <thead class="bg-slate-50/80"><tr>
                <th>Count No</th><th>Branch</th><th>Location</th><th>{{ __('ui.date') }}</th><th>Items</th><th>{{ __('ui.status') }}</th><th class="text-right">{{ __('pages.table.action') }}</th>
            </tr></thead>
            <tbody>
                ...
                @empty
                    <tr><td colspan="7">
                        <x-ui.empty-state title="{{ __('pages.empty.stock_counts') }}" description="{{ __('pages.empty.stock_counts_hint') }}">
