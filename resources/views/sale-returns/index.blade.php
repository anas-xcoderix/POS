@php $title = __('modules.sale_returns'); @endphp
<x-erp-layout>
    ...
    <div class="overflow-x-auto">
        <table class="erp-table min-w-full">
            <thead class="bg-slate-50/80"><tr>
                <th>Return No</th><th>{{ __('ui.customer') }}</th><th>{{ __('ui.date') }}</th><th>{{ __('ui.total') }}</th><th>{{ __('ui.status') }}</th><th class="text-right">{{ __('pages.table.action') }}</th>
            </tr></thead>
            <tbody>
                ...
                @empty
                    <tr><td colspan="6">
                        <x-ui.empty-state title="{{ __('pages.empty.sale_returns') }}" description="{{ __('pages.empty.sale_returns_hint') }}">
