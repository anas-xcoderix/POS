@php $title = __('modules.customer_statement').' — '.localized($customer); @endphp
<x-erp-layout>
<div class="space-y-6">
    <div class="erp-card p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">{{ $customer->code }} · Balance: {{ number_format($customer->balance, 2) }}</p>
            </div>
            <a href="{{ route('payments.create', ['party_type' => 'customer']) }}" class="erp-btn-primary shrink-0">
                <x-ui.icon name="plus" class="h-4 w-4" /> Record Receipt
            </a>
        </div>
