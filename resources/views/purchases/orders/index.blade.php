@php $title = __('modules.purchase_orders'); @endphp
<x-erp-layout>
<div class="erp-card overflow-hidden">
    <div class="flex flex-col gap-4 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <form method="GET" class="relative flex-1 sm:max-w-sm">
            ...
            <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('pages.search.po') }}" class="erp-input !mt-0 pl-10">
        </form>
    ...
    <div class="overflow-x-auto">
        <table class="erp-table min-w-full">
            <thead class="bg-slate-50/80"><tr>
                <th>PO No</th><th>{{ __('ui.vendor') }}</th><th>{{ __('ui.date') }}</th><th>{{ __('ui.total') }}</th><th>{{ __('ui.status') }}</th><th class="text-right">{{ __('pages.table.action') }}</th>
            </tr></thead>
            <tbody>
                ...
                @empty
                    <tr><td colspan="6"><x-ui.empty-state title="{{ __('pages.empty.purchase_orders') }}" description="{{ __('pages.empty.purchase_orders_hint') }}" /></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
