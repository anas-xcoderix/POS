@php $title = __('modules.dashboard'); @endphp
<x-erp-layout>
    {{-- Page header row --}}
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-sm text-slate-500">{{ __('pages.dashboard.subtitle') }}</p>
        </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-5 xl:grid-cols-12">
        <div class="erp-hero-card xl:col-span-7">
            <div class="relative z-10 max-w-lg">
                <p class="text-sm font-medium text-slate-500">{{ __('pages.dashboard.welcome_back') }}</p>
                <h2 class="mt-1 text-2xl font-bold text-slate-900 sm:text-3xl">{{ auth()->user()->name }}</h2>
                <p class="mt-3 text-sm text-slate-600">
                    {{ __('pages.dashboard.total_parts') }}: <strong class="text-slate-900">{{ number_format($stats['parts']) }}</strong>
                    <span class="mx-2 text-slate-300">|</span>
                    {{ __('pages.dashboard.stock_lines') }}: <strong class="text-orange-600">{{ number_format($stats['stock_lines']) }}</strong>
                    <span class="mx-2 text-slate-300">|</span>
                    {{ __('pages.dashboard.customers') }}: <strong class="text-slate-900">{{ number_format($stats['customers']) }}</strong>
                </p>
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="{{ route('sales-invoices.index') }}" class="erp-btn-primary">{{ __('pages.dashboard.sales') }}</a>
                    <a href="{{ route('parts.index') }}" class="erp-btn-dark">
                        <x-ui.icon name="plus" class="h-4 w-4" /> {{ __('pages.dashboard.add_part') }}
                    </a>
                </div>
            </div>
        ...
        <div class="erp-feature-card xl:col-span-5">
            <div class="border-b border-slate-100 px-5 py-4">
                <div class="flex items-center justify-between">
                    <h3 class="font-bold text-slate-900">{{ __('pages.dashboard.top_customer') }}</h3>
                    <a href="{{ route('customers.index') }}" class="text-sm font-medium text-orange-600 hover:text-orange-700">{{ __('pages.dashboard.view_all') }}</a>
                </div>
            </div>
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                                <div class="rounded-lg bg-slate-50 px-3 py-2">
                                    <p class="text-slate-500">{{ __('pages.dashboard.orders') }}</p>
                                    <p class="font-semibold text-slate-800">{{ $featured->orders_count ?? 0 }}</p>
                                </div>
                                <div class="rounded-lg bg-slate-50 px-3 py-2">
                                    <p class="text-slate-500">{{ __('pages.dashboard.type') }}</p>
                                ...
                                <div class="col-span-2 rounded-lg bg-slate-50 px-3 py-2 sm:col-span-1">
                                    <p class="text-slate-500">{{ __('pages.dashboard.total_customers') }}</p>
            ...
            @else
                <div class="p-8 text-center text-sm text-slate-500">
                    <p>{{ __('pages.dashboard.no_customer_sales') }}</p>
                    <a href="{{ route('sales-invoices.create') }}" class="mt-3 inline-flex text-orange-600 font-medium">{{ __('pages.dashboard.create_first_sale') }}</a>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <div class="erp-stat-card">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-slate-500">{{ __('pages.dashboard.total_revenue') }}</p>
                    ...
                    <span class="mt-2 inline-flex items-center gap-1 text-sm font-semibold {{ $stats['revenue_growth'] >= 0 ? 'text-emerald-600' : 'text-red-500' }}">
                        {{ $stats['revenue_growth'] >= 0 ? '+' : '' }}{{ $stats['revenue_growth'] }}% {{ __('pages.dashboard.last_month') }}
                    </span>
                </div>
        ...
        <div class="erp-stat-card">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-slate-500">{{ __('pages.dashboard.sales_invoices') }}</p>
                    ...
                    <span class="mt-2 inline-flex items-center gap-1 text-sm font-semibold {{ $stats['orders_growth'] >= 0 ? 'text-emerald-600' : 'text-red-500' }}">
                        {{ $stats['orders_growth'] >= 0 ? '+' : '' }}{{ $stats['orders_growth'] }}% {{ __('pages.dashboard.last_month') }}
                    </span>
                </div>
        ...
        <div class="erp-stat-card sm:col-span-2 xl:col-span-1">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-slate-500">{{ __('pages.dashboard.purchase_orders') }}</p>
                    <p class="mt-2 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{{ number_format($stats['purchase_orders']) }}</p>
                    <span class="mt-2 text-sm text-slate-500">{{ number_format($stats['purchase_invoices']) }} {{ __('pages.dashboard.invoices') }}</span>
                </div>
    ...
    {{-- Quick links --}}
    <div class="mb-6">
        <h3 class="mb-3 text-base font-bold text-slate-900">{{ __('pages.dashboard.quick_links') }}</h3>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            @foreach([
                [__('modules.sales_invoices'), 'document', route('sales-invoices.index')],
                [__('modules.parts_master'), 'box', route('parts.index')],
                [__('modules.purchase_orders'), 'cart', route('purchase-orders.index')],
                [__('modules.customers'), 'users', route('customers.index')],
            ] as [$label, $icon, $url])
    ...
    <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
        <div class="erp-card p-5 sm:p-6 xl:col-span-8">
            <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                <h3 class="text-base font-bold text-slate-900 sm:text-lg">{{ __('pages.dashboard.weekly_sales') }}</h3>
                <div class="flex flex-wrap items-center gap-3 text-xs sm:text-sm">
                    <span class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-orange-500"></span> {{ __('pages.dashboard.posted') }}</span>
                    <span class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-amber-300"></span> {{ __('pages.dashboard.draft') }}</span>
                </div>
            </div>
        ...
        <div class="erp-card p-5 sm:p-6 xl:col-span-4">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-base font-bold text-slate-900 sm:text-lg">{{ __('pages.dashboard.todays_revenue') }}</h3>
                <span class="erp-badge-orange">{{ __('pages.dashboard.live') }}</span>
            </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div class="erp-card overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                <h3 class="font-bold text-slate-900">{{ __('pages.dashboard.recent_sales') }}</h3>
                <a href="{{ route('sales-invoices.index') }}" class="text-sm text-orange-600 hover:text-orange-700">{{ __('pages.dashboard.view_all_lower') }}</a>
            </div>
                ...
                @forelse($recentSales as $sale)
                    <div class="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex items-center justify-between gap-3 sm:flex-col sm:items-end">
                            ...
                            <span class="erp-badge {{ $sale->status === 'posted' ? 'erp-badge-green' : 'erp-badge-amber' }}">{{ $sale->voided_at ? __('ui.voided') : ($sale->status === 'posted' ? __('ui.posted') : __('ui.draft')) }}</span>
                        </div>
                    </div>
                @empty
                    <div class="p-6"><x-ui.empty-state :title="__('pages.empty.sales_yet')" :description="__('pages.empty.sales_yet_hint')" /></div>
                @endforelse
            </div>
        </div>
        ...
        <div class="erp-card overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                <h3 class="font-bold text-slate-900">{{ __('pages.dashboard.recent_purchases') }}</h3>
                <a href="{{ route('purchase-invoices.index') }}" class="text-sm text-orange-600 hover:text-orange-700">{{ __('pages.dashboard.view_all_lower') }}</a>
            </div>
            <div class="divide-y divide-slate-100">
                @forelse($recentPurchases as $purchase)
                    <div class="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex items-center justify-between gap-3 sm:flex-col sm:items-end">
                            ...
                            <span class="erp-badge {{ $purchase->status === 'posted' ? 'erp-badge-green' : 'erp-badge-amber' }}">{{ $purchase->voided_at ? __('ui.voided') : ($purchase->status === 'posted' ? __('ui.posted') : __('ui.draft')) }}</span>
                        </div>
                    </div>
                @empty
                    <div class="p-6"><x-ui.empty-state :title="__('pages.empty.purchases_yet')" :description="__('pages.empty.purchases_yet_hint')" /></div>
                @endforelse
            </div>
        </div>
    </div>
