...
<head>
    ...
    <style>
        body { font-family: DejaVu Sans, sans-serif; font-size: 12px; color: #1e293b; direction: rtl; text-align: right; }
        ...
        table.items th, table.items td { border: 1px solid #e2e8f0; padding: 8px; text-align: right; }
        table.items th { background: #fff7ed; }
        .num { text-align: left; direction: ltr; }
        .total { margin-top: 16px; font-size: 14px; font-weight: bold; text-align: left; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <table class="items">
        <thead>
            <tr>
                ...
                <th class="num">{{ __('pdf.qty') }}</th>
                <th class="num">{{ __('pdf.unit_price') }}</th>
                <th class="num">{{ __('pdf.line_total') }}</th>
            </tr>
        </thead>
        <tbody>
    ...
    @endif
    <p style="margin-top:16px;color:#64748b;font-size:10px;">{{ __('pdf.thank_you') }}</p>
</body>
</html>
