@php $title = __('finance.customer_aging'); @endphp
<x-erp-layout>
<div class="mb-4 flex justify-between gap-3">
    <p class="text-sm text-slate-600">{{ __('finance.customer_aging') }}</p>
...
<div class="erp-card overflow-hidden">
    <div class="overflow-x-auto">
        <table class="erp-table min-w-full">
            <thead><tr>
                <th>{{ __('finance.customer') }}</th><th class="text-right">{{ __('finance.days_0_30') }}</th><th class="text-right">{{ __('finance.days_31_60') }}</th><th class="text-right">{{ __('finance.days_61_90') }}</th><th class="text-right">{{ __('finance.days_over_90') }}</th><th class="text-right">{{ __('finance.total_due') }}</th>
            </tr></thead>
            <tbody>
                @forelse($records as $row)
                    <tr>
                        <td>
                            <span class="font-semibold">{{ localized($row['customer']) }}</span>
                ...
                @empty
                    <tr><td colspan="6"><x-ui.empty-state :title="__('finance.no_outstanding')" /></td></tr>
                @endforelse
            </tbody>
            @if($records->isNotEmpty())
                ...
                <tfoot class="bg-slate-50 font-bold">
                    <tr>
                        <td class="text-right">{{ __('finance.totals') }}</td>
