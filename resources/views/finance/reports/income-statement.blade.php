@php $title = __('finance.income_statement'); @endphp
<x-erp-layout>
<div class="space-y-4">
    ...
    <div class="erp-card p-6 max-w-xl">
        <h3 class="mb-4 text-lg font-bold text-slate-900">{{ __('finance.income_statement') }}</h3>
        ...
        <dl class="space-y-4">
            <div class="flex justify-between border-b border-slate-100 pb-3">
                <dt class="text-slate-600">{{ __('finance.revenue') }}</dt>
                <dd class="font-semibold text-emerald-600">{{ number_format($revenue, 2) }}</dd>
            </div>
            <div class="flex justify-between border-b border-slate-100 pb-3">
                <dt class="text-slate-600">{{ __('finance.expenses') }}</dt>
                <dd class="font-semibold text-red-600">{{ number_format($expenses, 2) }}</dd>
            </div>
            <div class="flex justify-between pt-2">
                <dt class="text-lg font-bold text-slate-900">{{ __('finance.net_income') }}</dt>
