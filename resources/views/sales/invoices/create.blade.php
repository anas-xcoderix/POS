...
<x-erp-layout>
<form method="POST" action="{{ route('sales-invoices.store') }}" class="space-y-6">
    @csrf

    <div class="erp-card p-6">
        <h3 class="mb-4 flex items-center gap-2 text-base font-bold text-slate-900">
            <span class="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-100 text-sm text-indigo-700">1</span>
            Invoice Details
        </h3>
    ...
    <div class="erp-card overflow-hidden">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <h3 class="flex items-center gap-2 text-base font-bold text-slate-900">
                <span class="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-100 text-sm text-indigo-700">2</span>
                Line Items
            </h3>
    ...
    <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <a href="{{ route('sales-invoices.index') }}" class="erp-btn-secondary text-center">Cancel</a>
