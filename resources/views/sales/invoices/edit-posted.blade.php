    ...
    <div class="erp-card overflow-hidden">
        ...
        <div class="overflow-x-auto p-4">
            <table class="erp-table min-w-full">
                <thead><tr><th>Part</th><th>Location</th><th>Qty</th><th>Price</th><th>Disc %</th><th>VAT %</th><th></th></tr></thead>
                <tbody id="itemsBody">
                    @foreach($invoice->items as $idx => $item)
                        <tr>
                            ...
                            <td><input type="number" step="0.01" name="items[{{ $idx }}][discount_percent]" value="{{ $item->discount_percent }}" class="erp-input !mt-0 w-20"></td>
                            <td><input type="number" step="0.01" name="items[{{ $idx }}][vat_percent]" value="{{ $item->vat_percent }}" class="erp-input !mt-0 w-20"></td>
...
<template id="rowTemplate">
    <tr>
        ...
        <td><input type="number" step="0.01" name="items[__INDEX__][vat_percent]" value="0" class="erp-input !mt-0 w-20"></td>
        <td><button type="button" onclick="this.closest('tr').remove()" class="erp-btn-danger !p-2">×</button></td>
    </tr>
</template>
<script>
